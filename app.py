...
ss.setdefault("question_start", None)
...
if st.button("Toggle Dark/Light Theme"): ss.theme="dark" if ss.theme=="light" else "light"
if ss.theme=="dark": st.markdown("<body class='dark'>", unsafe_allow_html=True)
...
def generate_certificate(name,quiz_name,score,total,points):
    ...
    pdf.set_line_width(2); color=(212,175,55); pdf.set_draw_color(*color)
    pdf.rect(10,10,width-20,height-20)
    if os.path.exists(LOGO_FILE): pdf.image(LOGO_FILE, x=width/2-30, y=20, w=60)
    pdf.set_font("Helvetica","B",26); pdf.ln(50); pdf.cell(0,20,"Certificate of Achievement",0,1,'C')
    pdf.set_font("Helvetica","B",22); pdf.cell(0,10,name,0,1,'C')
    pdf.set_font("Helvetica","",16); pdf.cell(0,10,"has successfully completed",0,1,'C')
    pdf.set_font("Helvetica","B",18); pdf.cell(0,10,quiz_name,0,1,'C')
    pdf.set_font("Helvetica","",14); pdf.cell(0,10,f"Score: {score}/{total} | Points: {points}",0,1,'C')
    percent=score/total*100; ctype="Bronze"
    if percent>=90: ctype="Gold"
    elif percent>=75: ctype="Silver"
    pdf.set_font("Helvetica","B",16); pdf.cell(0,10,f"Certificate Type: {ctype}",0,1,'C')
    pdf.cell(0,10,datetime.now().strftime("%d %B %Y"),0,1,'C')
    pdf.line(width-70,height-50,width-10,height-50)
    pdf.set_font("Helvetica","",12); pdf.text(width-68,height-45,"Authorized Signature")
    path=f"data/certificate_{uuid.uuid4()}.pdf"; pdf.output(path); return path

# ---------------------------
# STUDENT QUIZ PAGE WITH REAL-TIME TIMER
# ---------------------------
def student_quiz_page(quiz_id):
    if quiz_id not in quizzes: st.error("Invalid Quiz"); return
    quiz = quizzes[quiz_id]; q_time = quiz.get("question_time",60)
    st.title(f"📝 {quiz['name']}")
    st.info(f"Time per question: {q_time} sec")
    ...
    if not ss.quiz_started:
        ...
        if st.button("Start Quiz"):
            ...
            ss.quiz_started=True; ss.question_index=0; ss.answers={}
            ss.consecutive_correct=0; ss.points=0
        ...
        idx = ss.question_index; q = quiz["questions"][idx]
        elapsed = time.time() - ss.question_start; remaining = q_time - elapsed
        if remaining<=0:
            st.warning("⏳ Time Over for this question! Answer saved as selected.")
            ss.question_index += 1; ss.question_start=time.time()
            if ss.question_index >= len(quiz["questions"]): submit_quiz(quiz_id); return
            st.rerun()
        circular_timer(remaining, q_time)
        st.write(f"### Q{idx+1}/{len(quiz['questions'])}"); st.write(q["question"])
        ans=None; qtype=q.get("type","mcq")
        if qtype=="mcq": ans=st.radio("Select one:",q["options"],key=f"q{idx}")
        elif qtype=="truefalse": ans=st.radio("Select:",["True","False"],key=f"q{idx}")
        else: ans=st.text_input("Answer:",key=f"q{idx}")
        ss.answers[idx]=ans
        st.progress((idx+1)/len(quiz["questions"]))
        col1,col2=st.columns(2)
        with col1:
            if idx>0 and st.button("⬅ Previous"): ss.question_index-=1; ss.question_start=time.time(); st.rerun()
        with col2:
            if idx<len(quiz["questions"])-1 and st.button("Next ➡"): ss.question_index+=1; ss.question_start=time.time(); st.rerun()
            elif idx==len(quiz["questions"])-1 and st.button("Submit Quiz"): submit_quiz(quiz_id)

# ---------------------------
# QUIZ SUBMISSION WITH GAMIFICATION & NEGATIVE MARKING
# ---------------------------
def submit_quiz(quiz_id):
    quiz=quizzes[quiz_id]; score=0; ss.consecutive_correct=0; ss.points=0
    for i,q in enumerate(quiz["questions"]):
        user_ans=str(ss.answers.get(i,"")).strip().lower()
        correct_ans=str(q["answer"]).strip().lower()
        if user_ans==correct_ans: score+=1; ss.consecutive_correct+=1; ss.points+=10+ss.consecutive_correct*2
        else: ss.consecutive_correct=0; ss.points-=2
    rid=str(uuid.uuid4())
    results[rid]={"name":ss.name,"regno":ss.regno,"quiz_id":quiz_id,"score":score,
                  "total":len(quiz["questions"]),"points":ss.points,"date":str(datetime.now()),
                  "answers":ss.answers}
    ...
    with open(cert_path,"rb") as f: st.download_button("🎖 Download Certificate",f,"certificate.pdf")
    st.balloons(); ss.quiz_started=False
...
# ROUTER
# ---------------------------
params=st.experimental_get_query_params(); quiz_id=params.get("quiz",[None])[0]
