...
def submit_quiz(quiz_id):
    ...
    ss.points = 0
    negative_enabled = quiz.get("negative", True)
...
def student_quiz_page(quiz_id):
    ...
    if not ss.quiz_started:
        ...
        ans = None
        if qtype == "mcq":
            ans = st.radio("Select one:", q["options"], index=q["options"].index(ss.answers.get(idx)) if ss.answers.get(idx) in q["options"] else 0, key=f"q{idx}")
        elif qtype == "truefalse":
            ans = st.radio("Select:", ["True","False"], index=["True","False"].index(ss.answers.get(idx)) if ss.answers.get(idx) in ["True","False"] else 0, key=f"q{idx}")
        else:
...
def admin_panel():
    ...
    with tabs[0]:
        ...
        negative = st.checkbox("Enable Negative Marking", value=True)
        if st.button("Create Quiz"):
            qid = str(uuid.uuid4())
            quizzes[qid] = {"name":qname, "time_limit":tlimit, "questions":[], "negative":negative}
        ...
        if quizzes:
            ...
            ans = ""
            if qtype=="mcq":
                opts = st.text_input("Options (comma separated)").split(",")
    ...
    with tabs[1]:
        ...
        for qid,q in quizzes.items():
            if query.lower() in q["name"].lower():
                ...
                st.code(url)
                if st.button("Delete", key=f"del_{qid}"): del quizzes[qid]; save_json(QUIZ_FILE, quizzes); st.experimental_rerun()
